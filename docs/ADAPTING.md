...
- **A different oceanographic monitoring site**: change `SITE_LAT`, `SITE_LON`, `SITE_NAME`,
  ...
  (`scripts/fetch_copernicus*.py`) and `app.py` read these values; nothing else needs to change
  for the physical-oceanography side of the pipeline.
- **A different bioassay spreadsheet with the same column schema** (`ID, DATE, EC50, UL, LL,
  ...
  fetch in `app.py` read from this single constant.
...
- **A different set of environmental variables** (add/remove a Copernicus variable beyond SST,
  salinity, O2, pH, CO2): update `ENV_COLS` in [`analysis/common.py`](../analysis/common.py)
...
`app.py` (tab labels, chart titles, tooltips), every module in `analysis/`, and
`scripts/mhw_lag_analysis.R`. There is no shortcut around this: a research group adapting the
framework to a different indicator should expect to:

1. Rename the indicator column consistently from `data_extended.csv`/`data_ec50_ci.csv`
   onward, updating `ALL_COLS` in `analysis/common.py`.
2. Update the statistical assumptions where they are indicator-specific (e.g. the rolling-mean
   imputation window in `analysis/common.py::load_data()`, the ARDL/CCF lag range `TAU_MAX` in
   the same file, chosen for *P. lividus*'s ~2-month gametogenesis cycle).
3. Update `app.py` labels/titles and the R script's variable names.
